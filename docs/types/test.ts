...
// The function returns a number...
{
	...
	mskrange( x.length, new AccessorArray( x ), 1, new AccessorArray( mask ), 1 ); // $ExpectType number
}
...
// Attached to main export is an `ndarray` method which returns a number...
{
	...
	mskrange.ndarray( x.length, new AccessorArray( x ), 1, 0, new AccessorArray( mask ), 1, 0 ); // $ExpectType number
}
